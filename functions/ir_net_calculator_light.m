function [spikes, conns] = ir_net_calculator_light(parameters, seed, network, V_m)
    %_________
    ...
    %   V_m = An [n x 1] vector of membrane potential for each 
    %               neuron at the first timestep
    %   G_in = An [n x t_steps+1] matrix of input conductance for each
    %               neuron at each timestep
    %
    %OUTPUTS:
    %   spikes = A structure containing spike times for each neuron
    ...
    for t = 1:parameters.t_steps
        ...
        spikers_bin = V_m >= parameters.V_th; %binary vector
        if t > ceil(parameters.init_period/parameters.dt) %If in the simulation interval
            spikers_ind = find(spikers_bin);
            for s_i = 1:length(spikers_ind)
                spikes(spikers_ind(s_i)).times = [spikes(spikers_ind(s_i)).times, t];
            end
        end
        ...
        G_sra(spikers_bin) = G_sra(spikers_bin) + parameters.del_G_sra; %set SRA conductance values
